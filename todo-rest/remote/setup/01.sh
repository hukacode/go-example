...
# Prompt to enter a password for the PostgreSQL (rather than hard-coding
# a password in this script).
read -p "Enter password for todoapp DB user: " DB_PASSWORD
...
apt update apt --yes -o Dpkg::Options::="--force-confnew" upgrade
...
sudo -i -u postgres psql -d todoapp -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d todoapp -c "CREATE ROLE todoapp WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the todoapp database to the system-wide environment
# variables in the /etc/environment file.
echo "GREENLIGHT_DB_DSN='postgres://todoapp:${DB_PASSWORD}@localhost/todoapp'" >> /etc/environment
